        ...
        <header className='h-16 shadow-md bg-white fixed w-full top-0 right-0 left-0 z-10'>
            ...
            <Container >
                ...
                <div className='flex justify-between items-center transition-all duration-1000 ease-in-out'>
                        ...
                        <ul className='flex items-center justify-between gap-6 font-semibold'>
                            ...
                            {navItem.map((nav) => (
                                ...
                                <li key={nav.title} className=''>
                                    <NavLink to={nav.link} className={({isActive}) => isActive ? 'text-primary hover:underline hover:underline-offset-4' 
                                    : 'text-secondary hover:underline hover:underline-offset-4 transition-all duration-300 delay-150 ease-in-out'} >
                                        {nav.title}
                                    </NavLink>
                                </li>
                            )
                    ...
                    {/* mobile menu */}
                    {isMobileMenuOpen && (
                        <div className='border border-secondary md:hidden absolute right-0 top-16 
                        bg-white w-full py-4 px-6 rounded-br-2xl rounded-bl-2xl transition-all duration-300 
                        ease-in-out'>
                            
                            <ul className='text-secondary flex items-start justify-between gap-2 font-semibold 
                            flex-col'>
                                {navItem.map((nav) => ( 
                                    <li key={nav.title}>
                                        <NavLink to={nav.link} className={({isActive}) => isActive ? 'text-primary' 
                                        : 'text-secondary'} onClick={() => SetisMobileMenuOpen(false)}>
                                            {nav.title}
                                        </NavLink>
                                    </li>
                                    )
